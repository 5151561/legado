...
class PreferenceCategory(context: Context, attrs: AttributeSet) :
    ...
    override fun onBindViewHolder(holder: PreferenceViewHolder) {
        ...
        if (view is TextView) {  //  && !view.isInEditMode
            ...
            view.setTextColor(context.legadoComponentTokens().preference.trailing)
            ...
            if (da is View) {
                da.setBackgroundColor(
                    context.legadoComponentTokens().preference.divider.takeIf { !AppConfig.isEInkMode }
                        ?: dividerColor
                )
